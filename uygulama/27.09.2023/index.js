...
let baslangicSayisi= parseInt(prompt("başlangıç sayısını giriniz"));
let bitisSayisi= parseInt(prompt("Bitiş sayısını giriniz"))
let toplam=0;
...
for(let ogrenci=1; ogrenci<=ogrenciSayisi; ogrenci++){
    let not= Number(prompt(ogrenci + ". ogrencinin notunu girini"))
    toplamNot += not
...
let tahmin = Number(prompt("Lüfen 1 ile 10 arasında bir sayı giriniz."))
let hak = 3;
let deneme = 1;
...
while(sayi !== tahmin && hak > 1){
    if (tahmin > sayi) {
        ...
        tahmin = Number(prompt(`Yanlış tahmin! Sayıyı artırın! Kalan hakkınız ${--hak}`))
}
deneme++;
}
